...
estHT_srd<-(N/Nd)*htestimate(ysd,N,PI,pk,method='yg')$mean
estHT_srd
...
### Wartość estymatora p-średniego Błędu Szacunku 

eD.ht_estHT_srd<-(N/Nd)*htestimate(ysd,N,PI,pk,method='yg')$se
eD.ht_estHT_srd
...
### wartość estymatora p-Średniego względnego błąd szacunku

egamma.ht_estHT_srd<-100*eD.ht_estHT_srd/estHT_srd
egamma.ht_estHT_srd

## INTEPRETACJA - szacuje się że ocena średniego względnego błędu 
## szacunku stanowi 59,76% wartości estymatora wartości średniej zmiennej y w domenie 7.
...
## zmiennej y w 7 domenie wynosi 225582,1.

# wartość estymatora p-średniego błędu szacunku estymatora

eD.estHT_wgd<-N*htestimate(ysd,N,PI,pk,method='yg')$se
eD.estHT_wgd
...
## INTEPRETACJA - szacuje się że ocena średniego względnego błędu 
## szacunku stanowi 59,75% wartości estymatora wartości globalnej zmiennej y w domenie 7.